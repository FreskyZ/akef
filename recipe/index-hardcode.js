...
const originalItemOrder = [...pagedata.items];
const elements = {
...
function drawRecipeTree(root) {
    ...
    function createNode(item, path) {
        ...
        j(itemElement, 'div', { className: 'image' }, e => {
        ...
        if (item.possibleProducts.length) {
            ...
            for (const [product, productIndex] of item.possibleProducts.map((p, i) => [p, i])) {
                ...
                j(productElement, 'div', { className: 'image' }, e => setupImageElement(e, product));
    ...
    function createLine(item, path, relativeSpeed) {
        ...
        j(lineElement, 'div', { className: 'image' }, e => setupImageElement(e, item.data, 32));
        ...
        const fillRecipeLine = (recipeId) => {
            ...
            for (const [{ id: itemId, count }, index] of recipe.ingredients.map((i, index) => [i, index])) {
                ...
                j(recipeLineElement, 'div', { className: 'image' }, e => setupImageElement(e, item, 32));
...
function updateItemList() {
    ...
    if (sortMethod == 'normal') {
        items.sort((i1, i2) => originalItemOrder.findIndex(o => o.name == i1.dataset['id']).localeCompare(originalItemOrder.findIndex(o => o.name == i2.dataset['id'])));
    }
    else {
        items.sort((i1, i2) => {
